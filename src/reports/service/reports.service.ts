...
import { ReportGrade, ReportSubject, StudentReport } from "../types/reports.types";
...
export class ReportsService {
  ...
  async parseReportsCSV(file: Express.Multer.File): Promise<Array<Student>> {
    ...
    try{
        ...
        const reportSubjects:Array<ReportSubject> = lines[0]?.substring(1)?.split(',')?.map((subject) => ({name:subject}));

        const subjectEntities = await this.subjectsService.upsertMany(reportSubjects,transaction);


        let studentReports:Array<StudentReport> = [];
        ...
        studentsLines.forEach((line) =>
        {
          ...
          let gradesData: Array<ReportGrade> = [];

          for(let i = 0; i < reportSubjects.length; i++){
            gradesData.push({
              value: grades[i] === '' ? null : Number(grades[i]),
              subjectId: subjectEntities.find((entity) => entity.name === reportSubjects[i].name)?.id
            })
          }
